...
To start the console npw call:
<code>npm i & npm run start</code>
or
<code>yarn install & yarn start</code>
dc-cli start - запускает локальную сеть на машине
затем, yarn install & yarn start
таким образом проверяем поднята ли локальная сеть на своем комьютере
